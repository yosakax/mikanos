...
/** @brief 指定されたPCIデバイスの指定されたケーパビリティレジスタを読み込む
 *
 * @params dev ケーパビリティを読み込むPCIデバイス
 * @ params addr ケーパビリティレジスタのコンフィグレーション空間アドレス
 * */
CapabilityHeader ReadCapabilityHeader(const Device &dev, uint8_t addr);

/** @brief MSIケーパビリティ構造
 *
 * MSIケーパビリティ構造は，６４ビットサポートの有無などで亜種がたくさんある
 * この構造体は各亜種に対応するために最大の亜種に合わせてメンバを定義してある
 * NOTE: bitsで無名構造体を作るとビット単位で名前をつけられる
 * */
 ...
 * @param num_vector_exponent 割り当てるベクタ数(2^n のnを指定)
 * */
